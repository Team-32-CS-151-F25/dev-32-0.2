...
public class EditStudentController {
    ...
    private void populateFields(){
        ...
        if ("Employed".equals(student.getJobStatus()) ) {
            ...
            jobDetailsTextField.setDisable(false);
            jobDetailsTextField.setText(student.getJobDetails());
        } else if ("Unemployed".equals(student.getJobStatus())) {
            unemployed.setSelected(true);
        }

        studentEvaluationArea.setText(student.getEvaluation());

        for(CheckBox language : languages)
            language.setSelected(student.getProgrammingLang().contains(language.getText()));

        for(CheckBox db : database)
            db.setSelected(student.getDatabases().contains(db.getText()));
